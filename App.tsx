...
import { supabase, isSupabaseConfigured } from './supabase'; // Added imports
...
// Keeping your original keys for fallback, plus intro seen key
const STORAGE_KEYS = {
...
interface LoginScreenProps {
  onLogin: (name: string, email: string, avatar: string) => Promise<void>; // Kept for compat
  onSuccess: () => void; // Added for Auth flow
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, currentLang, setLang, onSuccess, isLoading }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured || !supabase) {
        // Fallback to legacy mode if no DB
    ...
    try {
        if (mode === 'signup') {
            ...
            // 2. Create Profile (Using 'any' cast to prevent build errors with strict types)
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {mode === 'signup' && (
                    <div className="animate-in slide-in-from-top-1 relative">
                        ...
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-xl pl-11 pr-4 py-3 font-semibold text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors" placeholder={t.nameLabel} />
                    </div>
                )}
                
                <div className="relative">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-xl pl-11 pr-4 py-3 font-semibold text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors" placeholder={t.emailLabel} />
                </div>
                
                <div className="relative">
                    <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                    <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-xl pl-11 pr-12 py-3 font-semibold text-white placeholder-slate-500 pr-12 focus:outline-none focus:border-blue-500 transition-colors" placeholder={t.passwordLabel} />
...
const App: React.FC = () => {
  ...
  // Keep isAuthLoading separate or merge with loading
  ...
  // This usersDb is for the Leaderboard
      ...
      if (!hasSeen) {
          ...
          if (videoUrl) {
              ...
              try {
                  ...
                  console.warn("Could not save video seen flag.");
              }
          }
      }
      setLanguage(code);
  ...
  // UPDATED: INITIAL DATA LOAD & CLEANUP (Supports Supabase)
  useEffect(() => {
    const initData = async () => {
      ...
      if (isSupabaseConfigured && supabase) {
          ...
          // No DB configured, falling back to local (Legacy Mode)
                ...
                setTeamsData(prev => {
                    ...
                    Object.keys(rankMap).forEach(teamId => {
                        if (next[teamId]) {
                            ...
                            next[teamId] = { ...next[teamId], rank: newRank };
                        }
                    });
                    return next;
                });
            ...
            if (dbMatches && dbMatches.length > 0) {
               setMatches((prev: Match[]) => {
                  const updatedMatches: Match[] = prev.map((m) => {
                     ...
                     if (realM) {
                        return { ...m, ...realM }; // Merging props roughly
                     }
                     return m;
                  });
  ...
  const fetchUserProfile = async (email: string) => {
      ...
      try {
          ...
          if (data) {
              ...
              // Handle League Invites after fetch
      ...
      } catch (err) { console.error("Fetch profile failed", err); } finally { setIsAuthLoading(false); }
  };

  // Bracket Logic (Dependent on user)
  useEffect(() => {
    if (user && !isAdminMode) {
        setMatches(prev => {
  ...
  // Legacy local storage sync (keeping for safety during transition)
  useEffect(() => {
    if (!isDataLoaded) return;
    const safeSet = (key: string, val: string) => {
        try { localStorage.setItem(key, val); } catch (e) { /* ignore quota */ }
    };
    if (user) safeSet(STORAGE_KEYS.CURRENT_USER, JSON.stringify(user));
  }, [user, isDataLoaded]);

  // Handle Legacy Login (Still used by LoginScreen prop types, though we switched logic inside)
  // This is kept empty because LoginScreen handles the Supabase call internally now.
  const handleLegacyLogin = async (name: string, email: string, avatar: string) => {};
  ...
  const handleRefreshBracket = async () => {
    // Keep implementation but reload handles auth check
  ...
  const handleScoreUpdate = async (matchId: string, h: number, a: number) => {
    ...
    if (isSupabaseConfigured && supabase) {
       const { error } = await supabase.from('predictions').upsert({ 
          ...
          home: Number(h), 
          away: Number(a) 
       } as any, { onConflict: 'user_id,match_id' });

       if (error) {
           console.error("Autosave failed:", error);
           addToast('error', 'Save Failed', 'Connection issue?');
       }
    }
  };
  ...
  const handleClearPredictions = useCallback(async () => {
    if (!user || !supabase) return; 
    try {
        const q = supabase.from('predictions').delete().eq('user_id', user.email);
        if (activeTab === 'groups') {
           ...
           await q.in('match_id', knockoutIds);
        }
  ...
  if (isAuthLoading) return <div className="h-screen bg-[#05101c] flex items-center justify-center text-white font-black tracking-widest text-xs animate-pulse">CONNECTING TO STADIUM...</div>;

  // Replaced manual login check with Session check
  if (!session || !user) return <LoginScreen onLogin={handleLegacyLogin} onSuccess={() => {}} currentLang={language} setLang={handleLanguageSwitch} isLoading={isAuthLoading} />;
            ...
            <div className="flex flex-col h-full animate-fade-in">
                ...
                {tournamentSubTab === 'bracket' && (
                    <KnockoutBracket 
                      ...
                      onTeamClick={handleTeamClick} 
                      // FIX: Connected Spy props to avoid red lines
