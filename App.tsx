...
import { TournamentSchedule } from './components/TournamentSchedule'; // CORRECT IMPORT
...
const App: React.FC = () => {
  ...
  // Group matches by date for groupedMatches view
  ...
  useEffect(() => {
    ...
    const safeSet = (key: string, val: string) => {
        try { localStorage.setItem(key, val); } catch (e: any) { console.error(`Storage Error`, e); }
    };
  ...
  const handleAuth = async (name: string, email: string, avatar: string) => {
    ...
    if (isSupabaseConfigured && supabase) {
        try {
            const { data: profileData } = await supabase.from('profiles').select('*').eq('email', safeEmail).single();
            if (profileData) {
                remoteProfile = {
                    name: profileData.name, email: profileData.email, tokens: profileData.tokens,
                    substitutions: profileData.substitutions || 5, unlockedMatches: profileData.unlocked_matches || [],
                    favorites: profileData.favorites || [], avatar: profileData.avatar, 
                    hasTakenSecondChance: profileData.has_taken_second_chance || false,
                    spiedMatches: profileData.spied_matches || [], leagues: profileData.leagues || []
                };
            }
            const { data: predData } = await supabase.from('predictions').select('*').eq('user_id', safeEmail);
            if (predData && predData.length > 0) {
                remotePreds = (predData as any[]).map((p: any) => ({
                    userId: p.user_id, matchId: p.match_id, home: p.home, away: p.away
                }));
            }
        } catch (err) { console.error("Error fetching remote data", err); }
    }
    let activeUser: UserProfile;
    if (remoteProfile) { activeUser = remoteProfile; setUsersDb(prev => ({ ...prev, [safeEmail]: activeUser })); } 
    else if (usersDb[safeEmail]) { activeUser = usersDb[safeEmail]; } 
    else {
        const initialLeagues = pendingLeague ? [pendingLeague] : [];
        activeUser = { name: safeName, email: safeEmail, tokens: 5, substitutions: 5, unlockedMatches: [], favorites: [], avatar: avatar || '😊', hasTakenSecondChance: false, spiedMatches: [], leagues: initialLeagues };
        setUsersDb(prev => ({ ...prev, [safeEmail]: activeUser }));
        if (isSupabaseConfigured && supabase) {
           await supabase.from('profiles').upsert({ 
               email: safeEmail, name: safeName, avatar, tokens: 5, substitutions: 5, unlocked_matches: [], has_taken_second_chance: false, leagues: initialLeagues, spied_matches: [], favorites: []
           }, { onConflict: 'email' });
        }
    }
    if (remotePreds.length > 0) {
        setAllPredictions(prev => { const others = prev.filter(p => p.userId !== safeEmail); return [...others, ...remotePreds]; });
    }
    if (pendingLeague) {
        ...
        if (!currentLeagues.includes(pendingLeague)) {
            ...
            setUsersDb(prev => ({ ...prev, [safeEmail]: activeUser }));
            if (isSupabaseConfigured && supabase) await supabase.from('profiles').update({ leagues: activeUser.leagues }).eq('email', safeEmail);
        }
        sessionStorage.removeItem('pending_league_invite');
        addToast('success', 'League Joined', `Welcome to the ${pendingLeague.toUpperCase()} league!`);
    }
    ...
    if (!document.querySelector('.text-red-400') && !document.querySelector('.text-amber-400')) addToast('success', t.welcome, `Good luck, ${activeUser.name}!`);
    if (!localStorage.getItem(`rasten_first_login_${STORAGE_KEYS.USERS}`)) { setShowRules(true); localStorage.setItem(`rasten_first_login_${STORAGE_KEYS.USERS}`, 'true'); }
  };
  ...
  const updateAvatar = async (newAvatar: string) => {
    ...
    if (isSupabaseConfigured && supabase) await supabase.from('profiles').update({ avatar: newAvatar }).eq('email', user.email);
    setShowAvatarEditor(false);
    addToast('success', 'Profile Updated', 'New avatar looks great!');
  };

  const handleLogout = () => { setIsLoggedIn(false); setUser(null); setIsProfileMenuOpen(false); localStorage.removeItem(STORAGE_KEYS.CURRENT_USER); addToast('info', 'Logged Out', 'See you next match day.'); };
  ...
  const handleUnlockSecondChance = async () => {
     ...
     if (window.confirm("Are you sure? This will reduce all future points by 50%.")) {
         ...
         setUser(updatedUser);
         setUsersDb(prev => ({ ...prev, [user.email]: updatedUser }));
         if (isSupabaseConfigured && supabase) await supabase.from('profiles').update({ has_taken_second_chance: true }).eq('email', user.email);
  ...
  const handleSubstitute = async (matchId: string) => {
      ...
      const updatedUser = { ...user, substitutions: user.substitutions - 1, unlockedMatches: [...(user.unlockedMatches || []), matchId] };
      setUser(updatedUser);
      setUsersDb(prev => ({ ...prev, [user.email]: updatedUser }));
      if (isSupabaseConfigured && supabase) await supabase.from('profiles').update({ substitutions: updatedUser.substitutions, unlocked_matches: updatedUser.unlockedMatches }).eq('email', user.email);
      addToast('success', t.subSuccess, `${t.substitutions}: ${updatedUser.substitutions} left`);
  };

  const handleConfirmSub = async (matchId: string) => {
      if (!user) return;
      const newUnlocked = (user.unlockedMatches || []).filter(id => id !== matchId);
      const updatedUser = { ...user, unlockedMatches: newUnlocked };
      setUser(updatedUser);
      setUsersDb(prev => ({ ...prev, [user.email]: updatedUser }));

      if (isSupabaseConfigured && supabase) {
          await supabase.from('profiles').update({ unlocked_matches: newUnlocked }).eq('email', user.email);
      }
      addToast('success', 'Locked', 'Change saved and match locked.');
  };

  const handleSpy = async (matchId: string) => {
      if (!user) return;
      if (user.tokens < 1) { addToast('error', 'No Intel', 'You need more Intel to spy.'); return; }
      const updatedUser = { ...user, tokens: user.tokens - 1, spiedMatches: [...(user.spiedMatches || []), matchId] };
      setUser(updatedUser);
      setUsersDb(prev => ({ ...prev, [user.email]: updatedUser }));
      if (isSupabaseConfigured && supabase) await supabase.from('profiles').update({ tokens: updatedUser.tokens, spied_matches: updatedUser.spiedMatches }).eq('email', user.email);
  ...
  const hasGroupPredictions = useMemo(() => {
    ...
    return allPredictions.some(p => p.userId === user.email && matches.some(m => m.id === p.matchId && m.groupId));
  }, [allPredictions, user, matches]);

  const hasKnockoutPredictions = useMemo(() => {
    if (!user) return false;
    return allPredictions.some(p => p.userId === user.email && matches.some(m => m.id === p.matchId && m.round && m.round !== 'R32'));
  }, [allPredictions, user, matches]);
  ...
  const handleClearPredictions = useCallback(async () => {
    ...
    } catch (err) { console.error("Failed to clear predictions:", err); addToast('error', 'Error', 'Failed to clear predictions.'); }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-32 md:pb-12">
            ...
            <div className="bg-[#0f2545] border-b border-white/5 py-3 shadow-inner overflow-x-auto no-scrollbar">
                <div className="flex gap-2 px-4 justify-start sm:justify-center">
                    {GROUP_CONFIG.map(g => {
                        ...
                        const userPredsCount = allPredictions.filter(p => 
                            p.userId === user?.email && 
                            groupMatches.some(gm => gm.id === p.matchId)
                        ).length;
                        ...
                        let statusClasses = 'bg-slate-600'; // Default Gray
                        if (isComplete) statusClasses = 'bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.8)]';
                        else if (inProgress) statusClasses = 'bg-orange-500 shadow-[0_0_6px_rgba(249,115,22,0.8)]';
                        ...
                        return (
                            <button key={g.id} onClick={() => { setActiveGroup(g.id); setShowOverview(false); }} className={`relative min-w-[64px] h-16 rounded-xl overflow-hidden transition-all duration-300 transform active:scale-95 border-2 ${isActive ? 'scale-110 border-yellow-400 z-10 shadow-2xl' : 'border-white/10 hover:border-white/30'}`}>
                                ...
                                <div className={`absolute bottom-1.5 right-1.5 w-2.5 h-2.5 rounded-full border border-black/30 ${statusClasses}`}></div>
                            </button>
                        );
                    })}
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
                   ...
                   <div className="bg-slate-200 p-1 rounded-xl flex gap-1 shadow-inner border border-slate-300">
                      {(['schedule', 'tables', 'bracket'] as const).map(sub => (
                         <button key={sub} onClick={() => setTournamentSubTab(sub)} className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${tournamentSubTab === sub ? 'bg-[#0f2545] text-white shadow-md' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-300/50'}`}>
                            {sub === 'schedule' && <CalendarDays size={14} />} {sub === 'tables' && <ListOrdered size={14} />} {sub === 'bracket' && <GitMerge size={14} />}
        ...
        {activeTab === 'groups' && tournamentPhase === 'PRE_LIVE' && (
            <div {...swipeHandlers} className="animate-fade-in touch-pan-y">
                {showOverview ? (
                   ...
                   <>
                      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6"><StandingsTable standings={standings} teams={teamsData} lang={t} onTeamClick={handleTeamClick} /></div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {groupMatchesList.map(match => (
                              <MatchCard 
                                key={match.id} match={match} homeTeam={teamsData[match.homeTeamId]} awayTeam={teamsData[match.awayTeamId]} onUpdate={handleScoreUpdate} lang={t} locale={currentLocale} userTokens={user?.tokens || 0} rivals={rivalsList} onSpy={handleSpy} revealedRivals={[]} currentUser={user} allPredictions={allPredictions} phase={tournamentPhase} isAdminMode={isAdminMode}
                                onSubstitute={() => handleSubstitute(match.id)} substitutionsLeft={user?.substitutions || 0} isUnlockedBySub={user?.unlockedMatches?.includes(match.id) || false} onTeamClick={handleTeamClick} onConfirmSub={() => handleConfirmSub(match.id)}
                              />
                          ))}
                      </div>
                          ...
                          <div className="flex gap-3 w-full max-w-lg">
                              ...
                              {activeGroup !== 'L' ? <button onClick={handleNextGroup} className="flex-[2] px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-2xl shadow-lg font-black uppercase tracking-widest hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2 group"><span>Next Group</span><ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" /></button> : (
                                  <div className="flex-[2] flex gap-2">
                                      <button onClick={() => setShowOverview(true)} className="flex-1 px-4 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm text-blue-600 font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2"><LayoutGrid size={18} /> {t.tablesBtn}</button>
                                      <button onClick={() => setActiveTab('knockout')} className="flex-1 px-4 py-4 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-2xl shadow-lg font-black uppercase tracking-widest hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2">Bracket <ChevronRight size={18} /></button>
                                  </div>
                              )}
                          </div>
                      </div>
        ...
        {activeTab === 'manager' && (tournamentPhase === 'PRE_LIVE' ? 
            ...
            : 
            <MyPredictions 
                matches={matches} teams={teamsData} allPredictions={allPredictions} currentUser={user} lang={t} 
                onGoToGroup={handleGoToGroup} onGoToBracket={() => setActiveTab('knockout')} onUnlockSecondChance={handleUnlockSecondChance} 
                onSubstitute={handleSubstitute} onConfirmSub={handleConfirmSub}
            />
        )}
      </main>
      ...
      {showAvatarEditor && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Change Identity</h3><button onClick={() => setShowAvatarEditor(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button></div>
      ...
      <DebugTools 
        ...
        onSimulateGroups={() => { const simulated = simulateFullTournament(matches, teamsData, user?.favorites || [], 'GROUPS'); setMatches(simulated); addToast('success', 'Simulation Complete', 'Group stage simulated.'); }}
        onSimulateKnockouts={() => { const simulated = simulateFullTournament(matches, teamsData, user?.favorites || [], 'KNOCKOUT'); setMatches(simulated); addToast('success', 'Simulation Complete', 'Knockouts simulated.'); }}
        onClear={() => { localStorage.clear(); window.location.reload(); }} onTimeTravel={handleTimeTravel} isAdminMode={isAdminMode} onToggleAdmin={() => setIsAdminMode(!isAdminMode)} lang={t} users={Object.values(usersDb) as UserProfile[]} predictions={allPredictions} matches={matches}
      />
      <RulesModal isOpen={showRules} onClose={() => setShowRules(false)} lang={t} />
      {isHelpingHandOpen && user && (
          <HelpingHandModal 
            isOpen={isHelpingHandOpen} onClose={() => setIsHelpingHandOpen(false)} teams={teamsData} initialFavorites={user.favorites} 
            onGenerate={async (favs, scope) => {
                const simulatedMatches = simulateFullTournament(matches, teamsData, favs, scope);
                const newPredictions: Prediction[] = [];
                simulatedMatches.forEach(m => { if (m.homeScore !== null && m.awayScore !== null) { newPredictions.push({ userId: user.email, matchId: m.id, home: m.homeScore, away: m.awayScore }); } });
                setAllPredictions(prev => { const otherUsersPreds = prev.filter(p => p.userId !== user.email); const myOldPreds = prev.filter(p => p.userId === user.email); const mergedMyPreds = [...myOldPreds]; newPredictions.forEach(np => { const idx = mergedMyPreds.findIndex(p => p.matchId === np.matchId); if (idx >= 0) mergedMyPreds[idx] = np; else mergedMyPreds.push(np); }); return [...otherUsersPreds, ...mergedMyPreds]; });
                if (isSupabaseConfigured && supabase) { const payload = newPredictions.map(p => ({ user_id: p.userId, match_id: p.matchId, home: p.home, away: p.away })); if (payload.length > 0) await supabase.from('predictions').upsert(payload, { onConflict: 'user_id,match_id' }); }
                addToast('success', 'Magic Applied', `Simulated ${newPredictions.length} matches based on favorites.`);
            }} 
            lang={t} mode={activeTab === 'knockout' ? 'knockout' : 'groups'} 
          />
      )}
