...
import { Match, Team, GroupStanding, Round, Prediction, UserProfile, Translation, HeadToHeadStats, HistoricalMatch, MatchHistoryItem, ScoutingData } from '../types';
...
export const calculateMaxPotentialPoints = (matches: Match[], predictions: Prediction[], user: UserProfile): number => {
    ...
    matches.forEach(m => {
        ...
        if (isFinishedOrLive && m.homeScore !== null && m.awayScore !== null) {
            ...
            if (m.round) {
                 const baseVal = SCORING_RULES[m.round] || 5;
                 const penaltyRounds: Round[] = ['R32', 'R16', 'QF', 'SF', 'FIN', '3RD'];
...
export const calculateGroupStandings = (groupId: string, matches: Match[], teams: Record<string, Team>): GroupStanding[] => {
  ...
  const groupConfig = GROUP_CONFIG.find(g => g.id === groupId);
  
  if (groupConfig) {
      groupConfig.teams.forEach(tId => {
...
export const simulateFullTournament = (
    ...
    for (let i = 0; i < 7; i++) {
        const matchesToPredict = currentMatches.filter(m => {
...
export const simulateTournamentAtDate = (
    ...
    let simMatches = currentMatches.map(m => {
...
export const fetchHeadToHeadStats = async (homeTeam: Team, awayTeam: Team): Promise<HeadToHeadStats> => {
    ...
    const seed = homeTeam.id.charCodeAt(0) + awayTeam.id.charCodeAt(0);
    const mockMatches = [];
    let homeWins = 0;
    let awayWins = 0;
    let draws = 0;
    ...
    for(let i=0; i<5; i++) {
        const year = 2024 - (i * 2);
        const pseudoRand = Math.sin(seed + i) * 10000;
        const s1 = Math.floor((Math.abs(pseudoRand) % 4));
        const s2 = Math.floor((Math.abs(pseudoRand * 2) % 4));
        
        let winnerId: string | 'DRAW' = 'DRAW';
        if (s1 > s2) { winnerId = homeTeam.id; homeWins++; }
        else if (s2 > s1) { winnerId = awayTeam.id; awayWins++; }
        else draws++;
...
export const fetchTeamHistory = async (teamId: string): Promise<MatchHistoryItem[]> => {
  ...
  return Array.from({length: 5}).map((_, i) => {
      const res = ['W', 'D', 'L'][Math.floor(Math.random() * 3)] as 'W'|'D'|'L';
      return {
          opponent: 'OPP',
          result: res,
          score: res === 'W' ? '2-1' : res === 'L' ? '0-1' : '1-1',
          date: (2024 - i).toString()
      }
  });
};

export const fetchScoutingOverview = async (teamId: string): Promise<ScoutingData | null> => {
    if (!supabase) {
        console.warn("Scouting fetch: Supabase not configured");
        return null;
    }
    try {
        // Query by specific team_id (e.g. 'ARG') which is reliable
        const { data, error } = await supabase
        ...
        if (error) {
            console.error(`Scouting fetch error for ${teamId}:`, error);
            return null;
        }

        if (!data) {
            console.warn(`No scouting data found for team: ${teamId}`);
            return null;
        }

        return data;
    } catch (e) {
        console.error("Scouting fetch exception:", e);
        return null;
    }
};
