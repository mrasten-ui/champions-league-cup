...
export const LoginScreen: React.FC<LoginScreenProps> = ({ 
  ...
  const [riskResult, setRiskResult] = useState(50);
  const [riskScoring, setRiskScoring] = useState(50);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (mode === 'signup') {
            ...
            // Stash risk profile + avatar in sessionStorage BEFORE triggering auth, so
            // fetchUserProfile can read them even if onAuthStateChange fires during the
            // signUp await (the profiles row itself is created later, in useAppData.ts).
            sessionStorage.setItem('pending_risk_result', String(riskResult / 100));
            sessionStorage.setItem('pending_risk_scoring', String(riskScoring / 100));
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 text-white p-6 relative overflow-hidden">
       <div className="absolute top-[-10%] right-[-10%] w-96 h-96 bg-cyan-500/10 blur-[120px] rounded-full pointer-events-none"></div>
       <div className="absolute bottom-[-15%] left-[-10%] w-96 h-96 bg-fuchsia-500/10 blur-[120px] rounded-full pointer-events-none"></div>
       ...
       <div className="w-full max-w-md animate-fade-in z-10 flex flex-col items-center gap-8">
          <div className="text-center space-y-4 flex flex-col items-center">
              ...
              <h1 className="text-4xl font-black italic tracking-tighter uppercase bg-gradient-to-br from-white via-cyan-200 to-fuchsia-300 bg-clip-text text-transparent">The Rasten Cup '26</h1>
          ...
          <div className="w-full bg-blue-950/40 backdrop-blur-md rounded-3xl p-8 shadow-2xl text-slate-300 border border-white/10 ring-1 ring-white/5">
             <div className="mb-8 flex justify-center gap-4">
                  {LANGUAGES.map((lang) => (
                      <button key={lang.code} onClick={() => setLang(lang.code)} className={`relative w-14 h-10 rounded-md overflow-hidden transition-all duration-300 transform ${currentLang === lang.code ? 'scale-110 ring-2 ring-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.5)] grayscale-0' : 'opacity-50 grayscale hover:opacity-100 hover:scale-105'}`}>
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
                {errorMsg && <div className="bg-rose-500/10 border border-rose-500/50 rounded-xl p-3 text-rose-200 text-xs font-bold flex items-center gap-2 animate-in slide-in-from-top-2"><div className="bg-rose-500 rounded-full p-1"><X size={10} className="text-white" /></div>{errorMsg}</div>}
                {successMsg && <div className="bg-emerald-500/10 border border-emerald-500/50 rounded-xl p-3 text-emerald-200 text-xs font-bold flex items-center gap-2 animate-in slide-in-from-top-2"><div className="bg-emerald-500 rounded-full p-1"><CheckCircle2 size={10} className="text-white" /></div>{successMsg}</div>}
